        ...
        <div class="bg-white rounded-lg shadow-md">
            ...
            <div id="files-container" class="divide-y divide-gray-200">
                ...
                <div id="empty-state" class="px-6 py-12 text-center" style="display: none;">
                    ...
                    <a href="/sandbox/101-domain-rfx/101-screen-multi-file-upload" class="text-blue-600 hover:text-blue-800 font-medium">
                        파일 업로드하기
                    </a>
                </div>
            </div>
...
async function downloadFile(fileId) {
    try {
        ...
        if (file.download_url) {
            ...
            // API를 통한 다운로드 시도
            const response = await fetch(`/api/uploaded-files/${fileId}/download`, {
                headers: {
...
async function viewDocumentAssets(fileId) {
    try {
        ...
        if (result.success) {
            // 에셋 분석 결과 페이지로 이동
            const url = `/sandbox/101-domain-rfx/105-screen-document-analysis?file_id=${fileId}`;
