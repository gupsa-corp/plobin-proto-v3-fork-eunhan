...
/**
 * 샌드박스 템플릿 공통 설정 파일
 * TemplateCommonService 클래스를 사용한 깔끔한 접근
 */

// Bootstrap으로 autoload 로드
require_once __DIR__ . '/../../../bootstrap.php';

// TemplateCommonService 사용
...
function getCurrentScreenInfo() {
    return TemplateCommonService::getCurrentTemplateScreenInfo();
}
...
function getUploadPaths() {
    return TemplateCommonService::getTemplateUploadPaths();
}

/**
 * API 엔드포인트 URL 생성 (호환성 함수)
 */
function getApiUrl($endpoint = '') {
    return TemplateCommonService::getTemplateApiUrl($endpoint);
}

/**
 * 다른 화면으로의 URL 생성 (호환성 함수)
 */
function getScreenUrl($screenType, $screenName) {
    return TemplateCommonService::getTemplateScreenUrl($screenType, $screenName);
}
...
/**
 * 현재 화면에 대한 디버그 정보 출력 (호환성 함수)
 */
function debugCurrentLocation() {
    TemplateCommonService::debugTemplateLocation();
}
